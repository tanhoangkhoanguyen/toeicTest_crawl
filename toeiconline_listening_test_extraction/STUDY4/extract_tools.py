...
def extract_test_part1(soup, json_info, folder, id):
    save_img_path = extract_img(soup, "img", folder)
...
def extract_test_part6(soup, json_info, id):
    passage = ""
    if id % 4 > 2:
...
def extract_test(key):
    ...
    for question_tab in question_tabs:
        ...
        if i < 7:
            ...
            json_info = extract_test_part6(question_tab, json_info, i)
        else:
            break
...
def extract_answer(key):
    with open(f"crawled_html/{key}/raw_answer.html", encoding = "utf-8") as f:
        answer_soup = BeautifulSoup(f, "html.parser")