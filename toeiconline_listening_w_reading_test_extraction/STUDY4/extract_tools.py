...
DIRECTORY = "toeiconline_listening_w_reading_test_extraction/STUDY4/crawled_html"
...
def extract_audio(soup, key, folder):
    ...
    save_file_path = f"{DIRECTORY}/{folder}/audio/{file_name}"
...
def extract_img(soup, key, folder):
    ...
    if img_tag:
        ...
        if file_url == None:
            ...
            return ""
        file_name = os.path.basename(file_url)
        save_file_path = f"{DIRECTORY}/{folder}/img/{file_name}"
...
def extract_test(parts_number, key):
    with open(f"{DIRECTORY}/{key}/raw_test.html", encoding = "utf-8") as f:
    ...
    json_store_path = f"{DIRECTORY}/{key}/test_parser_output.json"
...
def extract_answer(parts_number, key):
    with open(f"{DIRECTORY}/{key}/raw_answer.html", encoding = "utf-8") as f:
        soup = BeautifulSoup(f, "html.parser")
    ...
    json_store_path = f"{DIRECTORY}/{key}/answer_parser_output.json"
    with open(json_store_path, "w", encoding = "utf-8") as f:
        print (json.dumps(json_info, indent = 4), file = f)